#!/usr/bin/env python3
"""
Multi-IP XSS Injection Attack Simulator
Simulates XSS injection attacks from different source IPs.
...
class MultiIPXSSSimulator:
    ...
    def run_sequential_attack(self, source_ips: List[str], payloads: List[str], 
        ...
        print(f"\n{'='*80}")
        print(f"🔴 SEQUENTIAL MULTI-IP XSS INJECTION ATTACK")
        print(f"{'='*80}")
    ...
    def run_parallel_attack(self, source_ips: List[str], payloads: List[str], 
        ...
        print(f"\n{'='*80}")
        print(f"🔴 PARALLEL MULTI-IP XSS INJECTION ATTACK (DDoS-style)")
        print(f"{'='*80}")
        print(f"  Source IPs: {len(source_ips)}")
        print(f"  Payloads per IP: {requests_per_ip}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Multi-IP XSS Injection Attack Simulator"
    )
    parser.add_argument(
        "--api-url",
    ...
    simulator = MultiIPXSSSimulator(args.api_url)
