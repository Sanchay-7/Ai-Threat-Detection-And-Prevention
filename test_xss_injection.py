#!/usr/bin/env python3
"""
XSS (Cross-Site Scripting) Testing Script
...
DATASET_PATH = "dataset/Large-Scale Annotated Dataset for Cross-Site Scripting (XSS) Attack Detection.csv"
BATCH_SIZE = 50
...
class XSSInjectionTester:
    ...
    def load_dataset(self) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"✅ Loaded {len(df)} XSS payloads from dataset")
            return df
    ...
    def get_attack_payloads(self, df: pd.DataFrame) -> List[str]:
        ...
        # Handle both 'Query' and 'Sentence' column names
        payload_col = 'Query' if 'Query' in df.columns else 'Sentence'
        df_copy = df.copy()
        df_copy['Label'] = pd.to_numeric(df_copy['Label'], errors='coerce')
        attacks = df_copy[df_copy['Label'] == 1][payload_col].dropna().astype(str).unique()
        attacks = [p for p in attacks if p and p.strip()]
        return list(attacks)
    ...
    def get_benign_payloads(self, df: pd.DataFrame) -> List[str]:
        ...
        df_copy = df.copy()
        df_copy['Label'] = pd.to_numeric(df_copy['Label'], errors='coerce')
        benign = df_copy[df_copy['Label'] == 0][payload_col].dropna().astype(str).unique()
        benign = [p for p in benign if p and p.strip()]
        return list(benign)
    ...
    def test_payload(self, payload: str) -> Tuple[bool, dict]:
        ...
        try:
            ...
            detected = result.get('xss', {}).get('decision', False)
            return True, {
                'payload': payload[:80],
                'detected': detected,
                'score': result.get('xss', {}).get('score', 0),
                'reason': result.get('xss', {}).get('reason', 'unknown')
            }
        except Exception as e:
    ...
    def run(self, test_sample_size: int = None):
        ...
        print(f"🚀 XSS Detector Testing Script")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="XSS Detection Testing Script"
    )
    parser.add_argument(
        "--api-url",
    ...
    parser.add_argument(
        "--dataset",
        default="dataset/Large-Scale Annotated Dataset for Cross-Site Scripting (XSS) Attack Detection.csv",
        help="Path to XSS dataset CSV"
    )
    parser.add_argument(
        "--sample",
        type=int,
    ...
    # Create tester
    tester = XSSInjectionTester(args.api_url, args.dataset)
