"""Train an XSS classifier (TF-IDF + Logistic Regression).
...
def train(dataset_path: str | None = None, model_path: str | None = None):
    ...
    out_path = Path(model_path) if model_path else Path(config.XSS_MODEL_PATH)
    ...
    print(f"Saved XSS model to {out_path}")
