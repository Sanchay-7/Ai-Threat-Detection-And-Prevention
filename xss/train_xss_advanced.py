...
def detect_gpu():
    ...
    if gpus:
        try:
            ...
            # Best-effort; continue even if memory growth setting fails.
            pass
    return bool(gpus)
...
def build_ensemble_model(X_train_tfidf, y_train, use_gpu=False):
    ...
    # Linear SVM + calibration gives probability estimates without heavy RBF cost.
    ...
    if use_gpu:
        # Prefer GPU for histogram algorithm; predictor hint for older/newer xgboost versions.
